...
ord = (num) => if(num > 3) "th" else ord_enum[num]
...
select = (cont, users) => {
  ...
  if(result.length
 ) result.reduce((a, b)=> if(a.length > b.length) a else b)
  else ""
}
...
most = (arr) => {
  counts = arr.reduce((a, c) => {
    a[c] = (if(a.hasOwnProperty(c)) a[c] else 0) + 1
    a
  }, Object())
  print(counts)
...
passJudge = () => {
  ...
  if(kill_mode
    ...
    if(werewolves.length && people.length) -1 // both alive
    else if(werewolves.length) 0 // werewolves alive
    else if(people.length) 1 // people alive
    else 2 // both zero, draw
  }
  else { // kill side
    if(werewolves.length && gods.length && villagers.length) -1 // both alive
    else if(werewolves.length) 0 // werewolves alive
    else if(gods.length && villagers.length) 1 // people alive
    else 2 // both zero, draw
  }
}
...
state prepare {
  ...
  event [msg, me] (user, cont: "^/go\\s*$") => {
    if((roles.length && names.length == roles.length)
      || (!roles.length && names.length in rolesMap)) {
      if(drrr.room.gameRoom && drrr.room.host == drrr.user.id) going initial
      else going prelude
    }
    else drrr.print(me(T("configNotFit")(
      if(roles.length) String(roles.length) else "5 - 12",
      String(names.length)
    )))
  }
    ...
    if(config) {
      ...
      if(!rs || !Array.isArray(rs))
        ...
        drrr.print("invalid roles: " + String(invalid)
          + "\n" + valid_roles)
      else if(!rs.filter(r => isWolf(r)).length)
        drrr.print("at least one werewolf")
      else if(rs.filter(r => r == rSeer).length > 1)
        drrr.print("seer cannot be more than one")
      // else if(rs.filter(r => r == rWitc).length > 1)
  ...
  event [msg, me] (user, cont: "^/setup\\s*") => {
    ...
    rstr = rstr + "\nkill mode: " + (if(kill_mode) "clear" else "side")
    rstr = rstr + "\nsave self: " + (if(save_self) "yes" else "no")
    drrr.print(rstr)
  }
...
state initial {
  ...
  drrr.getLoc(()=>{
    drrr.users.forEach((u, i) => {
     ...
     ) setTimeout(() => setAlive(u.name, true), cnt++ * 1500)
    })
...
state prelude {
  ...
  if(!villagers.length || !gods.length) kill_mode = 1
  ...
  later 2000 {
    ...
    forEach(players, (p, index) => {
      if(!isWolf(p.role)) later (index * 3500 + 1000) drrr.dm(p.name, T("UrRole")(p.rname), p.rUrl)
...
state night_seer {
  ...
  if(!roles.includes(theRole)) going night_guard

  announce(me(T("talkSeer")))
  if(filter(players, (p, idx) => p.role == theRole && p.life).length) {
    asked = 0

    forEach(players, (p, index) => {
      if(p.life && p.role == theRole)
      later 1500 drrr.dm(p.name, T("wantToKnow"))
    })

    event dm (seer, cont) => {
      if(seer in players) {
        if(!asked && players[seer].role == theRole) {
          if(players[seer].life) {
            if(!asked) {
              the = select(cont, names)
              if(the) {
                drrr.dm(seer, if(players[the].role > 0) T("people") else T("werewolf"))
                asked = 1
    ...
    event [dm, msg] (user, cont: "^/skip\\s*$") => {
      if(user in players) going night_guard
    }
...
state night_guard {
  ...
  if(!roles.includes(theRole)) going night_wolf
  ...
  if(filter(players, (p, idx) => p.role == theRole && p.life).length) {
    ...
    event dm (guard, cont) => {
      if(guard in players) {
        if(players[guard].role == theRole) {
          if(players[guard].life) {
            if(!used) {
              the = select(cont, names)
              if(cont.startsWith("no") || cont.startsWith("ignore")) {
                used = 1
                protect = ""
...
state night_wolf {
  ...
  forEach(players, (p, index) => {
    if(p.life && isWolf(p.role))
    later (2000 * index + 1500) drrr.dm(p.name, T("askKill"))
  })

  event dm (wolf, cont) => {
    if(wolf in players) {
      if(isWolf(players[wolf].role)) {
        if(players[wolf].life) {
  ...
  event [dm, msg] (user, cont: "^/skip\\s*$") => {
    if(user in players) going night_witch
  }
}
...
state night_witch {
  ...
  if(!roles.includes(theRole)) going night_end
  ...
  if(filter(players, (p, idx) => p.role == theRole && p.life).length) {
    ...
    names.forEach((name, index) => {
      poi_targets[name] = 0
      if((players[name].role == theRole) && players[name].life) {
        wait_list.push(name)
        players[name].used = false;
        if(players[name].poison)
    ...
    check_going = (witch) => {
      ...
      if(index >= 0) wait_list.splice(index, 1);
      if(!wait_list.length
     ) {
        if(med_counts < 3 && med_counts > 0) victim = []
        Object.keys(poi_targets).forEach(name => {
          if(poi_targets[name] % 2) {
    ...
    event dm (witch, cont) => {
      if(witch in players) {
        if(players[witch].role == theRole) {
          if(players[witch].life) {
            if(!players[witch].used) {
              the = select(cont, names)
              if(cont.startsWith("no")) {
                players[witch].used = true
                check_going(witch)
              } else if(cont.startsWith("ignore")) {
                players[witch].used = true
                check_going(witch)
              } else if(cont.includes("save")) {
                if(!players[witch].medicine) {
                  drrr.dm(witch, T("noMedicine"))
                }
                else if(((!save_self || (save_self && day > 1))
                  && victim.length && victim[0] == witch))
                  drrr.dm(witch, T("can'tSaveSelf"))
                else if(victim.length && victim[0] == protect) {
              ...
              } else if(cont.includes("poison")) {
                if(the.length) {
                  if(players[the].life){
                    if(players[witch].poison) {
...
state shooter_ask {
  forEach(players, (p, index) => {
    if(dead.includes(p.name) && isShooter(p.role))
    later (1500 * (index + 1)) drrr.dm(p.name, T("shooter15s"))
  })
  event [dm, msg, me] (hunter, cont) => {
    if(hunter in players && dead.includes(hunter)) {
      if(isShooter(players[hunter].role) && !expo.includes(hunter)) {
        expo.push(hunter)
        if(players[hunter].role == rHunt
       ) drrr.print(me(T("hunterShow")(hunter)))
        else if(players[hunter].role == rBing
       ) drrr.print(me(T("blackWolfShow")(hunter)))
      }
    }
  }
}
...
state idiot_ask {
  forEach(players, (p, index) => {
    if(dead.includes(p.name) && p.role == rIdio)
    later 2000 drrr.dm(p.name, T("idiot15s"))
  })
  event [dm, msg, me] (idiot, cont) => {
    if(idiot in players) {
      if(players[idiot].role == rIdio && dead.includes(idiot)) {
        ...
        if(index >= 0) dead.splice(index, 1);
...
state shooter_fire {
  ...
  event [msg, me, dm] (hunter, cont) => {
    ...
    if(hunter in players) {

      if(new_dead.includes(hunter)) {
        print("yes you are here")
        if(isShooter(players[hunter].role) && !new_expo.includes(hunter)) {
      ...
      else if(isShooter(players[hunter].role) && expo.includes(hunter)) {
        ...
        if(the) {
          if(players[the].life) {
            ...
            // what if(isShooter
            if(isShooter(players[the].role)) {
              // send asking info
              j = passJudge()
              if((isWolf(players[the].role)) && (j == 1)) going game_over
              if((!isWolf(players[the].role)) && (j == 0)) going game_over
            ...
            if(index >= 0) expo.splice(index, 1);
            check();
...
state night_end {
  ...
  later 3000 {
    if(victim.length) {
      ...
      if(victim.includes(protect)) {
        if(players[protect].diefor == "bite") {
          players[protect].diefor = ""
          index = victim.indexOf(protect);
          if(index >= 0) victim.splice(index, 1);
        }
      }
...
state day_discussion {
  ...
  event [msg, me] (u, cont) => {
    if((names[index] == u && cont.includes("/expo") && isWolf(players[u].role))) {
      if(players[u].role == rWing) {
        the = select(cont, names)
        if(the.length) {
          if(players[the].life){
            victim = [u]
            if(!victim.inclues(the)) victim.push(the)
      ...
      || ((names.includes(u) || u == drrr.user.name) && cont.startsWith("/skip"))) {
      index++ // += bug?
      while (index < names.length) && (!players[names[index]].life) index++;
...
state day_vote {
  ...
  event [msg, me, dm] (user, cont: "^/vote\\s+\\S+|^/execute", url, tc, req) => {
    ...
    if(user in players && !lock) {
      if(players[user].right) {
  ...
  event [msg, me, dm] (user, cont: "^/vote\\s*$", url, tc, req) => {
    if(req.type == "dm"
   ) drrr.dm(user, me(T("curVote")(Object.keys(vote).join(", "))))
...
state game_over {
  cur_role = map(players, (p, idx) => p.name + ":" + p.rname + " " + (if(p.life) T("alive") else (T("dead") + " (" + p.diefor + ")"))).join("\n")
  result = passJudge()
  if(result == 0
 ) scene(me(T("werewolvesWin")))
  else if(result == 1
 ) scene(me(T("peopleWin")))
...
werewolf = (lang) => {
  ...
  event [msg, me, dm] (user, cont: "^/r\\s*$") => {
    if(user in players) {
      wolves = filter(players, (p, index) => isWolf(p.role)).map(p => p.name).join(", ")
      if(!isWolf(players[user].role)) drrr.dm(user, T("UrRole")(players[user].rname), players[user].rUrl)
  ...
  event [msg, me, dm] (user, cont: "^/w\\s*$") => {
   ...
   ) drrr.print(T("players")(map(players, (p, index) => String(index + 1) + ". " + p.name + " " + (if(p.life) T("alive") else T("dead"))).join("\n")))
  ...
  event [msg, me] (user, cont: "^/alive") => {
    if(!names.length) drrr.print("/me start game first")
