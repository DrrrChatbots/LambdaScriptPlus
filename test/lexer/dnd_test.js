  ...
  msg.split("。").forEach(ctx => {
    if(ctx.length)
      chunkString(120, ctx + "。").forEach(chk => {
       ...
       ) { msgs.push(buffer); buffer = chk }
        else { buffer = buffer + chk; }
      })
  })
  if(buffer.length)) msgs.push(buffer)
  msgs.reverse();
...
state start {
  ...
  prev_question = () => {
    qs -= 1
    if(qs < 0
   ) { ch -= 1; qs = query[ch].questions.length - 1 }
    if(ch < 0
   ) { ch = 0; qs = 0; false }
    else true
  }
  ...
  ask = () => {
    if(qs == 0) drrr.print(query[ch].chapter)
...
state judge {
  ...
  Object.keys(alignments).forEach(attr => {
      if(alignments[attr].value == a) r += 1
  })
  if(r != 0) {
    pos = if(score.lx > score.nx && score.lx > score.cx) "l"
        else (if(score.cx > score.nx && score.cx > score.lx) "c" else "n")
    pos += if(score.xg > score.xe && score.xg > score.xn) "g"
        else (if(score.xe > score.xn && score.xe > score.xg) "e" else "n")
  }
