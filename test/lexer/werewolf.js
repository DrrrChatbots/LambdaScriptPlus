...
passJudge = () => {
  werewolves = filter(players, (p, index) => p.lif(e && isWolf(p.role))
  people = filter(players, (p, index) => p.lif(e && !isWolf(p.role))
  villagers = filter(players, (p, index) => p.lif(e && p.role == rVill)
  gods = filter(players, (p, index) => p.lif(e && isGod(p.role))
...
state night_seer {
  ...
  if( filter(players, (p, idx) => p.role == theRole && p.lif(e).length then {
    asked = 0

    forEach(players, (p, index) => {
      if( p.lif(e && p.role == theRole then
          ...
          if( players[guard].lif(e then {
            if( !used then {
              ...
              if( cont.startsWith("no") || cont.startsWith("ignore") then {
                ...
                else if( players[the].lif(e then{
                  protect = the
                  later 3500 going night_wolf
...
state night_witch {
  ...
  if( filter(players, (p, idx) => p.role == theRole && p.lif(e).length then {
    ...
    names.forEach((name, index) => {
      poi_targets[name] = 0
      if( (players[name].role == theRole) && players[name].lif(e then {
        wait_list.push(name)
...
state night_end {
  ...
  go_next = {
    drrr.print(me(T("survivors")(filter(players, p => p.lif(e).map(p => p.name).join(", "))))
    if( passJudge() < 0
...
state day_discussion {
  index = 0
  while (index < names.length) && (!players[names[index]].lif(e) index++;
...
state day_vote {
  ...
  vote_times++
  survivor = filter(players, (p, idx) => p.lif(e)
...
state day_execute {
  ...
  if( louis.length > 1 then {
    ...
    louis.forEach((name, index) => {
      ...
      setTimeout(() => setAlive(name, players[name].lif(e), (index + 1) * 1500)
    })
...
state game_over {
  cur_role = map(players, (p, idx) => p.name + ":" + p.rname + " " + (if( p.lif(e then T("alive") else (T("dead") + " (" + p.diefor + ")"))).join("\n")
  result = passJudge()
  if( result == 0
...
werewolf = (lang) => {
  ...
  event [msg, me, dm] (user, cont: "^/w\\s*$") => {
    ...
    then drrr.print(T("players")(map(players, (p, index) => String(index + 1) + ". " + p.name + " " + (if( p.lif(e then T("alive") else T("dead"))).join("\n")))
