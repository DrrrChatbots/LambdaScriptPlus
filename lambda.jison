...
LambdaParameterList
    : 'ident' ENDPAR_ARROW
    | 'ident' ',' LambdaParameterList
    | 'ident' ':' RightExpr ',' LambdaParameterList
    ;
...
ArrowExpr
    : LogicalORExpr
    | LambdaParameters ArrowExpr
    ;

ArrowExprNoIn
    : LogicalORExprNoIn
    | LambdaParameters ArrowExprNoIn
    ;
...
IfElse
  : 'if' '(' RightExpr ')' RightExpr %prec IF_WITHOUT_ELSE
  | 'if' '(' RightExpr ')' RightExpr 'else' RightExpr
  ;

Later : 'later' '(' RightExpr ')' RightExpr;
...
Iteration :
    'do' RightExpr 'while' '(' RightExpr ')' ';'
    | 'do' RightExpr 'while' '(' RightExpr ')' error
    | 'while' '(' RightExpr ')' RightExpr
    | 'for' '(' ExprNoInOpt ';' RExprOpt ';' RExprOpt ')' RightExpr
    /* | 'for' '(' VAR VariableDeclarationListNoIn ';' ExprOpt ';' ExprOpt ')' RightExpr */
    | 'for' '(' LeftHandSideExpr 'in' RightExpr ')' RightExpr
    | 'for' '(' LeftHandSideExpr 'of' RightExpr ')' RightExpr
    ;

Compound : '{' Exprs '}' ;

State : 'state' 'ident' RightExpr { $$ = 'a state' } ;
...
Expr
  : RightExpr Delimiter { }
  | RightExpr error { console.log("patched ; on RightExpr") }
  ;
